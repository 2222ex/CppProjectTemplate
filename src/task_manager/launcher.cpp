...
bool Launcher::launch_application(AppInfo &appInfo)
{
    ...
    Logger::Log()->info("get process {} pid: {}", appInfo.name, pid);
    if (pid != 0)
    {
    ...
    if (GetExitCodeProcess(appInfo.pi.hProcess, &appInfo.exit_code))
    {
...
bool Launcher::terminate_application(AppInfo &appInfo, std::string &err_msg)
{
    ...
    {
        SPDLOG_LOGGER_TRACE(log, "TerminateProcess {} success", appInfo.name);
        return true;
    }
