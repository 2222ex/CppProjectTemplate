...
void inner_work(int thread_id)
{
    try
    {
        ...
        if (!lock.try_lock())
        {
            SPDLOG_LOGGER_WARN(redis_log, "inner_work {}: failed to acquire lock", thread_id);
            return;
        }
        // 成功获得锁，安全地操作计数器
        ...
        SPDLOG_LOGGER_INFO(redis_log, "inner_work {}: counter = {}", thread_id, counter);
    }
    catch (const std::exception &e)
    {
        SPDLOG_LOGGER_ERROR(redis_log, "inner_work {}: Redis error: {}", thread_id, e.what());
    }
}
